...
当初は「ステージ数が10未満ならステージ数のみ、10以上ならJSONを出力する」といった制御をプロンプトだけで実現することも検討しました。しかし、出力形式が条件によって変化するとパース処理が複雑になり、想定外の出力による不具合のリスクも高まります。そのため、まずは必ず同じ形式のJSONを生成し、その結果をアプリケーション側で判定してモデルを切り替える設計を採用しました。
